...
def build_audio_graph(
    *,
    ...
    workflow.add_node(
        "analyze_semantics",
        functools.partial(analyze_semantics, inference=inference, emitter=emitter),
    )
...
def build_text_graph(*, inference: Any, emitter: Any = None) -> Any:
    """Build the text-only pipeline graph (no audio/cache).
